...
class Account(Base):
    ...
    mob_num  = Column('MobileNumber', String(50), CheckConstraint(sqltext = "MobileNumber REGEXP '^[+][:digit:]{3}[(][:digit:]{2}[)][:digit:]{3}-[:digit:]{2}-[:digit:]{2}$'", name = 'ch_mobnum'), unique = True)
    email    = Column('Email', String(50), CheckConstraint(sqltext = "Email REGEXP '^[:alpha:][[:alnum:]_]{5,15}[@][:alpha:]{2,10}[\.][:alpha:]{2,3}$'", name = 'ch_email'), unique = True)
