...
class Account(Base):
    ...
    mob_num  = Column('MobileNumber', String(50), CheckConstraint(sqltext = "MobileNumber REGEXP '^[+][:digit:]{3}[(][:digit:]{2}[)][:digit:]{3}-[:digit:]{2}-[:digit:]{2}$'", name = 'ch_mobnum'), server_default = '+999(99)999-99-99')
    email    = Column('Email', String(50), CheckConstraint(sqltext = "Email REGEXP '^[:alpha:][[:alnum:]_]{5,15}[@][:alpha:]{2,10}[\.][:alpha:]{2,3}$'", name = 'ch_email'), server_default = 'myemail@domen.com')
